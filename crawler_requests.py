...
class HouseholdCrawler:
    ...
    def __init__(self, use_ocr: bool = True):
        ...
        參數:
            use_ocr: 是否使用 OCR 自動識別驗證碼，預設為 True
        """
        ...
        self.captcha_key = ""
        self.city_code = ""
        self.current_token = ""  # 用於連續查詢的 token
    ...
    # ============================================================
    # 第五層 API (帶驗證碼重試): 查詢並自動處理驗證碼錯誤
    # ============================================================
    def query_with_captcha_retry(
        self,
        area_code: str,
        start_date: str,
        end_date: str,
        ...
        這個方法會自動處理驗證碼的取得和驗證，直到查詢成功或達到重試上限
        ...
        for attempt in range(1, max_retry + 1):
            ...
            if not captcha_input:
                ...
                return QueryResult(success=False, error_message="無法取得驗證碼")
        ...
        logger.error(f"連續 {max_retry} 次驗證碼錯誤，放棄")
        return QueryResult(success=False, error_message=f"連續 {max_retry} 次驗證碼錯誤")
...
def main(save_to_db: bool = True, save_to_csv: bool = True):
    ...
    if save_to_db and DB_AVAILABLE:
        ...
        print("         安裝方式: pip install pymysql")
